...
import { validateAndCalculateTotal, extractCommonDate } from './receipt-utils';
import { ReceiptValidationError } from './errors';
async function determineCategory(itemName) {
...
export function extractStoreInfo(lines) {
    const storeInfo = {
        storeName: 'Other', // Default to 'Other'
        storeAddress: ''
    };
    // Check if REWE appears anywhere in the receipt
    const isReweReceipt = lines.some(line => line.toUpperCase().includes('REWE') ||
...
export async function parseReweReceipt(text, receiptId) {
    try {
...
//# sourceMappingURL=rewe-parser.js.map