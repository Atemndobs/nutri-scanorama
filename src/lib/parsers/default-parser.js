...
export async function defaultReceiptParser(text, receiptId) {
    const receipt = {
...
export function extractItemsFromText(text) {
    try {
        ...
        const lines = text.split('\n').filter(line => line.trim() && !line.includes('---') && !line.toLowerCase().includes('category') && !line.toLowerCase().includes('price'));
