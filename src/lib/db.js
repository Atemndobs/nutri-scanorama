...
export class NutriScanDB extends Dexie {
        ...
        this.version(6).upgrade(tx => {
            return tx.receipts.toCollection().modify(receipt => {
                ...
                if (!receipt.purchaseDate)
                    receipt.purchaseDate = receipt.uploadDate;
    ...
    async deleteFailedScan(receiptId) {
    ...
    async deleteReceipt(receiptId) {
        await this.transaction('rw', [this.receipts, this.items, this.categories, this.receiptImages], async function () {
            // Get all items for this receipt
            const items = await this.items.where('receiptId').equals(receiptId).toArray();
            // Update category counts
            const categoryUpdates = items.reduce((acc, item) => {
                if (item.category) {
            ...
            await Promise.all(Object.entries(categoryUpdates).map(([category, count]) => this.categories
                .where('name')
                .equals(category)
                .modify(cat => {
    ...
    async determineCategory(itemName) {
    ...
    async recalculateCategoryCounts() {
        await this.transaction('rw', [this.items, this.categories], async () => {
            ...
            // Update category counts
            await Promise.all(Object.entries(counts).map(([category, count]) => this.categories
                .where('name')
                .equals(category)
                .modify(cat => {
                cat.itemCount = count;
