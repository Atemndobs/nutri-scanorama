...
export const UploadButton = () => {
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            ...
            // Perform OCR
            const result = await Tesseract.recognize(file, 'deu', // German language
            {
            ...
            <button onClick={() => handleAiExtraction(extractedText, receiptId)} disabled={isAiExtracting} className="bg-blue-500 text-white px-4 py-2 rounded mt-2">
            ...
            let displayErrorMessage = "Failed to process the receipt. Please try again.";
            ...
            if (error instanceof Error) {
                if (error.message.includes('No valid items')) {
                    ...
                    const lowerCaseText = extractedText.toLowerCase();
                    const hasKnownStore = ['rewe', 'lidl', 'aldi', 'nahkauf'].some(store => lowerCaseText.includes(store));
                    if (!hasKnownStore) {
                        const storeInfoContent = (<div className="space-y-2">
                ...
                <ul className="list-disc pl-4 space-y-1 text-sm">
                  ...
                  <li>Currently supported stores: REWE, Lidl, Aldi, Nahkauf</li>
                        ...
                        displayErrorMessage = isMobile ? (<div className="space-y-2">
    ...
    const handleAiExtraction = async (receiptText, receiptId) => {
        try {
    ...
    return (<div className="w-full space-y-2">
      <input type="file" ref={fileInputRef} onChange={handleFileUpload} accept="image/*" className="hidden" aria-label="Upload image" disabled={isUploading}/>
      <input type="file" ref={cameraInputRef} onChange={handleFileUpload} accept="image/*" capture="environment" className="hidden" aria-label="Take photo" disabled={isUploading}/>
      <div className="grid grid-cols-2 gap-2">
        <Button onClick={handleScan} disabled={isUploading} className="bg-gradient-to-r from-nutri-purple to-nutri-pink text-white hover:opacity-90 transition-opacity h-auto min-h-[2.5rem] w-full">
          {isUploading ? (<div className="flex items-center justify-center w-full">
        ...
        <Button onClick={handleClick} disabled={isUploading} className="bg-gradient-to-r from-nutri-purple to-nutri-pink text-white hover:opacity-90 transition-opacity h-auto min-h-[2.5rem] w-full">
          {isUploading ? (<div className="flex items-center justify-center w-full">
...
//# sourceMappingURL=UploadButton.jsx.map