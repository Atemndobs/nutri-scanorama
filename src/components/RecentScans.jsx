...
import { Receipt as ReceiptIcon, ShoppingBag, Wand2, Calendar, ShoppingCart, AlertTriangle, CheckCircle, Loader, Brain, Zap } from "lucide-react";
...
export const RecentScans = ({ className }) => {
    ...
    const loadReceiptWithItems = async (receipt) => {
    ...
    const receipts = useLiveQuery(async () => {
        ...
        const receiptsWithItems = await Promise.all(recentReceipts.map(loadReceiptWithItems));
        ...
        // Load items and categories for each receipt
        console.debug('[RecentScans] Loading items for receipts...');
        const receiptsWithItemsAndCategories = await Promise.all(receiptsWithItems.map(async (receipt) => {
            const items = await db.items
                .where('receiptId')
                .equals(receipt.id)
                .toArray();
    ...
    const handleViewItems = (receiptId) => {
    ...
    const handleManageCategory = (item) => {
    ...
    const handleAiExtraction = async (receipt) => {
        if (!receipt.id)
            return;
        try {
    ...
    return (<div className={`space-y-4 ${className}`}>
      {receipts?.receipts.length > 0 && (<>
          ...
          {receipts?.receipts.map((receipt) => (<Card key={receipt.id} className="bg-card/50 backdrop-blur-sm hover:bg-card/60 transition-colors cursor-pointer" onClick={() => handleReceiptClick(receipt)}>
              <CardContent className="flex items-center p-2">
                <div className="h-8 w-8 rounded-full bg-nutri-purple/10 flex items-center justify-center mr-3">
                  ...
                  {receipt.storeName ? (getStoreLogo(receipt.storeName) ? (<img src={getStoreLogo(receipt.storeName)} alt={receipt.storeName} className="h-6 w-6 object-contain"/>) : (<ReceiptIcon className="h-4 w-4 text-nutri-purple"/>)) : (<ReceiptIcon className="h-4 w-4 text-nutri-purple"/>)}
                </div>
                <div className="flex-1 space-y-0.5">
                    ...
                    >
                      {modelType === 'fast' ? (
                        ...
                        <Brain className="h-3 w-3 text-blue-500" />
                          ...
                          <Button variant="ghost" size="sm" className="h-5 px-1" onClick={(e) => {
                        e.stopPropagation();
                        ...
                        handleAiExtraction(receipt);
                            ...
                            <Badge variant="secondary" className="text-[10px] px-1 py-0 flex items-center gap-1">
                              {processingReceiptId === receipt.id ? (<Loader className="h-3 w-3 animate-spin text-purple-500"/>) : (<Wand2 className="h-3 w-3 text-purple-500"/>)}
                  ...
                  <div className="flex items-center space-x-1 text-xs text-muted-foreground">
                    ...
                    {receipt.processed && receipt.items && (<p className="flex items-center">
                ...
                <div className="text-right flex flex-col items-end">
                  ...
                  {receipt.processed && receipt.totalAmount && (<p className="font-medium">€{receipt.totalAmount.toFixed(2)}</p>)}
                </div>
              </CardContent>
            </Card>))}
        </>)}
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            year: '2-digit'
                    ...
                    {selectedReceipt?.discrepancyDetected && (<Badge variant="outline" className="flex items-center gap-1 text-muted-foreground">
                        <AlertTriangle className="w-3 h-3 text-destructive"/>
                        Items Missing
                      </Badge>)}
                    <div className="flex gap-2 items-center">
                      <Button variant="ghost" size="sm" disabled={isAiExtracting} onClick={() => {
            const newType = modelType === 'fast' ? 'precise' : 'fast';
            ...
            toast({
                ...
                className: newType === 'fast'
                    ...
                    : "bg-blue-500/10 border-blue-500/20",
                        ...
                        {modelType === 'fast' ? (<Zap className="h-3 w-3 text-yellow-500"/>) : (<Brain className="h-3 w-3 text-blue-500"/>)}
                      </Button>
                      <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => selectedReceipt && handleAiExtraction(selectedReceipt)} disabled={isAiExtracting}>
                        <Badge variant="secondary" className="text-[10px] px-2 py-0 flex items-center gap-1">
                          {isAiExtracting ? (<Loader className="h-3 w-3 animate-spin text-purple-500"/>) : (<Wand2 className="h-3 w-3 text-purple-500"/>)}
                          {isAiExtracting ? "Extracting..." : "Try AI "}
                        </Badge>
                      </Button>

                      {/* Add receipt thumbnail */}
                      {selectedReceipt?.id && (<div className="flex items-center">
                          <div className="cursor-pointer hover:opacity-80 transition-opacity flex items-center" onClick={handleImageClick}>
                            <ReceiptImageThumbnail receiptId={selectedReceipt.id} onClick={handleImageClick}/>
                          </div>
                        </div>)}
                    </div>
          ...
          {selectedReceipt && (<div className="space-y-6">
              <div className="space-y-4">
                <div>
                  <h3 className="text-xl font-semibold">{selectedReceipt.storeName}</h3>
                  {selectedReceipt.storeAddress && (<p className="text-sm text-muted-foreground mt-1">{selectedReceipt.storeAddress}</p>)}
                </div>
              ...
              {selectedReceipt.processed && selectedReceipt.items ? (<>
                  <div className="overflow-y-auto max-h-96">
                    ...
                    return (<TableRow key={index}>
                              ...
                              <TableCell>
                                <div className="flex items-center gap-2">
                                  {Icon && <Icon className="h-4 w-4" style={{ color: defaultCategories[item.category].color }}/>}
                                </div>
                              </TableCell>
                  ...
                  <div className="flex justify-end gap-2">
                    <Button variant="destructive" size="sm" onClick={() => handleDeleteReceipt(selectedReceipt.id)}>
                      Delete Receipt
                    </Button>
      ...
      {selectedReceipt?.id && (<ReceiptImageViewer receiptId={selectedReceipt.id} open={imageViewerOpen} onClose={() => {
...
//# sourceMappingURL=RecentScans.jsx.map