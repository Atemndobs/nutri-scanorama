...
export function CategoryManager() {
    ...
    const groupedMappings = categoryMappingsState.reduce((acc, mapping) => {
    ...
    if (!categoryMappingsState || !categories)
        return null;
    return (<div className="space-y-4">
      <div className="flex gap-2">
        <Input placeholder="Enter keyword..." value={newKeyword} onChange={(e) => setNewKeyword(e.target.value)}/>
      ...
      <div className="space-y-2">
        {Object.entries(groupedMappings).map(([category, mappings]) => (<Collapsible key={category} open={expandedCategories.has(category)} onOpenChange={() => toggleCategory(category)}>
            ...
            <CollapsibleContent className="space-y-1 mt-1">
              {mappings.slice(0, showAllItems ? undefined : PREVIEW_ITEMS).map((mapping) => (<div key={mapping.id} className="flex items-center justify-between p-2 ml-6 bg-card rounded-lg">
                  <span className="font-medium">{mapping.keyword}</span>
                  <Button variant="ghost" size="icon" onClick={() => mapping.id && handleDeleteMapping(mapping.id)}>
                    <Trash2 className="h-4 w-4"/>
                  </Button>
                </div>))}
              {!showAllItems && mappings.length > PREVIEW_ITEMS && (<Button variant="ghost" className="ml-6 text-sm" onClick={() => setShowAllItems(true)}>
      ...
      <div className="space-y-2 pt-4 border-t">
        ...
        <div className="space-y-2">
          <Input placeholder="Enter product names or descriptions to categorize..." value={poorlyExtractedText} onChange={(e) => setPoorlyExtractedText(e.target.value)}/>
          <Button onClick={handleCategorizeItems} disabled={isProcessing} className="w-full">
...
//# sourceMappingURL=CategoryManager.jsx.map