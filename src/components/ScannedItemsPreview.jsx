...
export const ScannedItemsPreview = () => {
      ...
      {items && items.length > 0 && (<>
          ...
          <div className="space-y-2">
            {items.map((item) => {
                ...
                return (<Card key={item.id} className="bg-card/50 backdrop-blur-sm hover:bg-card/60 transition-colors">
                  <CardContent className="p-2">
                    <div className="flex justify-between items-center">
                      <span>{capitalizeFirstLetter(item.name)}</span>
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                          {Icon && (<Icon className="h-4 w-4" style={{
                            color: defaultCategories[item.category || 'Other'].color
                        }}/>)}
...
//# sourceMappingURL=ScannedItemsPreview.jsx.map