    ...
    return (<>
      ...
      <div className="grid grid-cols-2 gap-4">
        {categories.slice(0, 4).map((category) => {
            ...
            return (<Card key={category.name} className="flex items-center p-4 gap-4 cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => setSelectedCategory({ name: category.name, color: category.color })}>
              {Icon && (<div className="shrink-0 flex flex-col items-center gap-1">
      ...
      {selectedCategory && (<CategoryItemsDialog category={selectedCategory.name} color={selectedCategory.color} isOpen={!!selectedCategory} onClose={() => setSelectedCategory(null)}/>)}
    </>);
};
//# sourceMappingURL=TopCategories.jsx.map