...
const ScansPage = () => {
    ...
    const receipts = useLiveQuery(async () => {
        ...
        const receiptsWithItems = await Promise.all(allReceipts.map(async (receipt) => {
    ...
    return (<div className="container mx-auto p-4">
      <div className="divide-y divide-muted-foreground">
        {receipts?.map((receipt) => (<Card key={receipt.id} className="bg-card/50 backdrop-blur-sm hover:bg-card/60 transition-colors cursor-pointer w-full border-none" onClick={() => handleReceiptClick(receipt.id)}>
            <CardContent className="p-4">
              <div className="flex-1 space-y-4">
                ...
                year: '2-digit'
                    ...
                    {receipt.processed ? (<CheckCircle className="w-3 h-3 text-green-500"/>) : (<Loader className="w-3 h-3 text-yellow-500 animate-spin"/>)}
                  </div>
                </div>

                {receipt.processed && receipt.items && (<div className="overflow-y-auto max-h-96">
                    <Table>
                      <TableHeader>
                        <TableRow>
...
//# sourceMappingURL=Scans.jsx.map