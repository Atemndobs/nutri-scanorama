...
export const ItemsPage = () => {
    ...
    const handleSave = async (itemId, formData = editForm) => {
        try {
    ...
    return (<div className="h-[calc(100vh-4rem)] overflow-hidden bg-background">
      <div className="container max-w-md mx-auto px-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex-1">
              <h1 className="font-semibold flex items-center whitespace-nowrap">
                {receipt?.storeName}
                {receipt?.storeAddress && (<span className="ml-2 text-xs text-muted-foreground truncate" style={{ maxWidth: '200px' }}>
            ...
            year: '2-digit'
                ...
                {thumbnailUrl && (<span className="cursor-pointer hover:opacity-80 transition-opacity flex items-center" onClick={() => setImageViewerOpen(true)}>
                    <img src={thumbnailUrl} alt="View receipt" className="w-5 h-5 object-cover rounded"/>
                  </span>)}
              </div>
            </div>
          </div>
        ...
        {parsedReceiptId && (<ReceiptImageViewer receiptId={parsedReceiptId} open={imageViewerOpen} onClose={() => {
            ...
            <TableBody>
              {items?.map((item) => (<TableRow key={item.id}>
                  {editingItemId === item.id ? (<>
                      <TableCell>
                        <Input value={editForm.name ?? capitalizeFirstLetter(item.name)} onChange={(e) => handleInputChange('name', e.target.value)} onKeyDown={(e) => handleKeyDown(e, item.id)}/>
                      </TableCell>
                      <TableCell>
                        <Select value={editForm.category ?? item.category} onValueChange={(value) => handleInputChange('category', value)}>
                          <SelectTrigger className="w-15 px-2">
                            <SelectValue>
                              {editForm.category && (<div className="flex items-center justify-center">
                        ...
                        style: { color: defaultCategories[editForm.category].color }
                          ...
                          <SelectContent align="start" className="min-w-[120px]">
                            {Object.entries(categoryIcons).map(([category, Icon]) => (<SelectItem key={category} value={category}>
                                <div className="flex items-center gap-2">
                                  {React.createElement(Icon, {
                        className: "h-4 w-4",
                        style: { color: defaultCategories[category].color }
                    })}
                      ...
                      <TableCell>
                        <Input type="number" value={editForm.price ?? item.price} onChange={(e) => handleInputChange('price', parseFloat(e.target.value))} onKeyDown={(e) => handleKeyDown(e, item.id)} step="0.01" min="0" className="w-16 text-right"/>
                      </TableCell>
                    </>) : (<>
                ...
                if (editingItemId === item.id) {
                    ...
                    setEditForm({});
                        ...
                        {editingItemId === item.id ? (<Save className="h-4 w-4"/>) : (<Pencil className="h-4 w-4"/>)}
                      </Button>
...
//# sourceMappingURL=Items.jsx.map